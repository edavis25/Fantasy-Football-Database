CREATE VIEW TeamGameView AS SELECT Teams.Name AS Name, 
...
CASE WHEN (Name LIKE GameView.Winner) THEN 'Y' ELSE 'N' END AS Won, 
CASE WHEN (Name LIKE GameView.Winner) THEN GameView.Loser ELSE GameView.Winner END AS Opponent, 
CASE WHEN (Name LIKE GameView.Winner) THEN GameView.WinScore ELSE GameView.LoseScore END AS TotalScore, 
CASE WHEN (Name LIKE GameView.Winner) THEN GameView.LoseScore ELSE GameView.WinScore END AS OppScore, 
...
TeamGame.URL AS URL 
FROM (Teams, TeamGame) 
LEFT JOIN GameView ON GameView.GameID = TeamGame.GameID WHERE TeamGame.TeamID = Teams.TeamID